// Data Structure AR/VR Visualization Functions for Practice Labs

// Map question types to data structures
...
function openDataStructureAR() {
    ...
    // Determine DS type from category
    ...
    overlay.style.cssText = 'position:fixed; inset:0; background:rgba(0,0,0,0.95); z-index:10001; display:flex; flex-direction:column; justify-content:space-between; padding:20px;';
    overlay.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center;">
            <h2 style="color:#e5e7eb; font-size:22px; margin:0; display:flex; gap:8px; align-items:center;">
                🕶️ ${dsInfo.name} AR Visualization - ${questionTitle}
            </h2>
        ...
        <div style="flex:1; display:grid; grid-template-columns:1fr 1fr; gap:20px; align-items:center;">
            <!-- 3D Canvas -->
            <div id="ds-ar-container" style="background:#111827; border:2px solid #334155; border-radius:12px; position:relative;">
                <canvas id="ds-ar-canvas" style="width:100%; height:100%; display:block; border-radius:10px;"></canvas>
            </div>
            <!-- Info Panel -->
            <div style="background:#0f172a; border:1px solid #1f2937; border-radius:12px; padding:20px; color:#cbd5e1;">
                ...
                <div style="background:#1e293b; padding:12px; border-radius:8px; margin-bottom:12px;">
                    ...
                    <pre style="margin:0; background:#0f172a; padding:8px; border-radius:6px; color:#10b981; font-size:11px; overflow-x:auto; white-space:pre-wrap; word-wrap:break-word; max-height:150px;">${code.substring(0, 200)}...</pre>
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px;">
                    <button onclick="rotateDS3DLeft()" style="background:#0ea5e9; color:#fff; border:none; padding:8px; border-radius:8px; cursor:pointer; font-size:12px; font-weight:600;">↶ Rotate</button>
                    <button onclick="zoomDS3DIn()" style="background:#22c55e; color:#0b172a; border:none; padding:8px; border-radius:8px; cursor:pointer; font-size:12px; font-weight:700;">🔍 Zoom</button>
                </div>
                <div style="margin-top:12px; padding:12px; background:#1e293b; border-radius:8px; border-left:3px solid ${dsInfo.color.toString(16).padStart(6, '0')};">
                    <p style="margin:0; font-size:12px; color:#cbd5e1;">
                        <strong>Type:</strong> ${dsInfo.name}<br>
                        <strong>Shape:</strong> ${dsInfo.shape}<br>
                        <strong>Color:</strong> #${dsInfo.color.toString(16).toUpperCase().padStart(6, '0')}
                    </p>
                </div>
            </div>
        </div>
...
function initDS3DVisualization(dsType, color) {
    ...
    // Create visualization based on DS type
...
function createDSGeometry(dsType, baseColor) {
    if (dsType === 'arrays') {
        ...
        // Create spheres connected by lines
        for (let i = 0; i < 4; i++) {
        ...
        const createNode = (x, y, depth) => {
            ...
            if (depth < 2) {
                ...
                [createNode(x - offset, nextY, depth + 1), createNode(x + offset, nextY, depth + 1)];
            }
        };
        createNode(0, 1.5, 0);
    } else if (dsType === 'hash') {
        ...
        // Create boxes stacked vertically
        for (let i = 0; i < 4; i++) {
